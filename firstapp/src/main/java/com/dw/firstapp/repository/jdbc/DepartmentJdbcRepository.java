...
public class DepartmentJdbcRepository implements DepartmentRepository {
    ...
    @Override
    public List<Department> saveDepartmentList(Department departmentList) {
        List<Department> departmentLists = new ArrayList<>();
        String query = "insert into 부서(부서명,부서번호) "
                + "values (?, ?)";
        try (
                Connection connection = DriverManager.getConnection(
                        URL, USER, PASSWORD);
            ...
            while (resultSet.next()) {
                ...
                department.setDepartmentNumber(resultSet.getString(departmentList.getDepartmentName()));
                department.setDepartmentName(resultSet.getString(departmentList.getDepartmentNumber()));
